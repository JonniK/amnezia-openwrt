...
setup() {
  ...
  # Redirect rc.d symlink operations to a temp dir so tests never touch /etc/rc.d.
  export AL_RCD="$BATS_TEST_TMPDIR/rc.d"
  mkdir -p "$AL_RCD"
}
...
@test "enable creates S97 boot symlink in rc.d" {
  run sh "$INIT" enable
  [ -L "$AL_RCD/S97amnezia-autolearn" ]
}
...
@test "disable removes S97 boot symlink from rc.d" {
  export UCI_DNSMASQ_logfacility="/tmp/dnsmasq-queries.log"
  sh "$INIT" enable
  # Symlink must exist after enable (-L tests for symlink, works even when dangling).
  [ -L "$AL_RCD/S97amnezia-autolearn" ]
  sh "$INIT" disable
  # Symlink must be gone after disable.
  [ ! -L "$AL_RCD/S97amnezia-autolearn" ]
}

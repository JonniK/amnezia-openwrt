...
setup() {
  ...
  # amnezia-failover-init, amnezia-dns-ctl are all stubs in PATH.
...
@test "master off: sets master_enabled=0, snapshots dot, stops daemon, flushes tables" {
...
@test "master on: deletes snapshot key after restore" {
  UCI_GET_amnezia_config_dot_master_saved=1 \
  run sh "$CTL" master on
  [ "$status" -eq 0 ]
